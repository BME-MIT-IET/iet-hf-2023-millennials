...
public class DefenceAgentTest {
    ...
    public void infectTest() {
        ...
        defAgent.infect(target, sender);
        verify(target, times(1)).addEffect(defAgent, sender);
        verify(target, times(1)).setEffectFlag(3);
    }
    ...
    public void tickTest() {
        ...
        int lifetime = defAgent.getLivetime();
        for (int i = 0; i < lifetime; i++) {
            assertEquals(lifetime - i, defAgent.getLivetime());
            iter.next();
            defAgent.tick(virologist, iter);
            assertEquals(lifetime - i - 1, defAgent.getLivetime());
        }
        assertEquals(0, defAgent.getLivetime());
